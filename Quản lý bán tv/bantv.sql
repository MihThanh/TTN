...
select HOADONBAN.MAHDBAN, NHANVIEN.HOTEN, KHACHHANG.TENKH, SANPHAM.TENSP,soluong, gtban from HOADONBAN, NHANVIEN, KHACHHANG, SANPHAM where HOADONBAN.MANV = NHANVIEN.MANV and HOADONBAN.MAKH = KHACHHANG.MAKH and HOADONBAN.MASP = SANPHAM.MASP and NGAYXUAT between '20171128' and '20171209' 
...
INSERT INTO HOADONBAN(MAHDBAN,MANV,MAKH,NGAYXUAT,MASP,SOLUONG,GTBAN) VALUES ('X1','1','K2','20171209','A1','1',''),
...
update HOADONBAN set MANV = '1', MAKH = 'K2',NGAYXUAT = '20171209', MASP = 'A1', SOLUONG= '1', GTBAN = '' where MAHDBAN = 'x1'
---proc tìm kiếm 
...
--PROC THÊM SP
create proc Themsp (@ma CHAR(10),@ten NCHAR(20),@gia INT ,@mansx char(10))
as
begin
INSERT INTO SANPHAM(MASP,TENSP,GIA,MANSX)
values(@ma,@ten,@gia,@mansx)
end

EXEC Themsp @ma='A2222',@ten=N'Samsung galaxy s9',@gia='455666',@mansx='s2'
select * from SANPHAM

select SANPHAM.MASP, tensp, SANPHAM.MANSX, TENNSX
from SANPHAM, NHASX
where SANPHAM.MANSX = NHASX.MANSX
...
begin
		...
		set	
			...
			MANSX=@mansx
			ngaynhap = @ngaynhap
		where MASP = @ma and SANPHAM.MASP = HOADONNHAP.MASP
end

exec xemsp

select SANPHAM.MASP, TENSP, SANPHAM.MANSX, TENNSX, HOADONNHAP.NGAYNHAP from SANPHAM, NHASX, HOADONNHAP where SANPHAM.MASP = HOADONNHAP.MASP and SANPHAM.MANSX = NHASX.MANSX
...
EXEC Them_nv @ma='12',@ten=N'thành',@ngaysinh='19960316',@sdt='23124213'
select * from NHANVIEN
...
EXEC Sua_nv @ma='12',@ten=N'Vu Minh Thanh',@ngaysinh='19960316',@sdt='124121123'
...
EXEC Them_ncc @ma='1',@ten='ss',@diachi='dsds',@sdt='9374844'
select * from nhacc
...
update NHACC set TENNCC = '', DIACHI = '', SDT = '' where MANCC = ''

EXEC Sua_ncc @ma='1',@ten='samsung',@diachi='sss',@sdt='214234234'

select * from NHACC where MANCC LIKE '%1%'

create proc xemsp
as
begin
	select SANPHAM.masp, tensp, sanpham.mansx, tennsx, gia, ngaynhap
	from sanpham, nhasx, HOADONNHAP
	where sanpham.mansx = nhasx.mansx and sanpham.masp = hoadonnhap.masp
end

exec xemsp

insert into NHANVIEN(MANV, HOTEN, NGAYSINH, SDT) values('1111', N'Minh Thanh', '20010316', '2243334')
